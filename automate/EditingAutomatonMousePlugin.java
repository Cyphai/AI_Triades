...
//import edu.uci.ics.jung.visualization.control.EditingGraphMousePlugin.ArrowPaintable;
//import edu.uci.ics.jung.visualization.control.EditingGraphMousePlugin.EdgePaintable;

public class EditingAutomatonMousePlugin extends EditingGraphMousePlugin implements
    ...
    public EditingAutomatonMousePlugin() {
     ...
     * @param modifiers
     */
    public EditingAutomatonMousePlugin(int modifiers) {
        super(modifiers);
    ...
    public void mousePressed(MouseEvent e) {
        if((e.getModifiersEx() &  MouseEvent.BUTTON1_DOWN_MASK) ==  MouseEvent.BUTTON1_DOWN_MASK)
        {
        	...
        	Automaton graph = (Automaton)vv.getGraphLayout().getGraph();
        	...
        	if(pickSupport != null) 
        	{
        		...
        		if(vertex != null) 
        		{ //Il y a un sommet à l'endroit où l'on a cliqué
        			if ((e.getModifiersEx() & MouseEvent.CTRL_DOWN_MASK) == MouseEvent.CTRL_DOWN_MASK)
        			{
        				//CTRL est enfoncé, on édite l'action du sommet

        				graph.editAction(vertex);
        			}
        			else
        			{
                ...
                {	 // S'il n'y a pas de sommet sous la souris et que shift est appuyé, on créé un nouveau sommet avec l'éditeur d'action à lui associer.
     ...
     * existing vertex, create an directed edge from startVertex to
     * the vertex under the mouse pointer with a transition. 
     */
        ...
        if(checkModifiers(e)) {
            ...
            if(pickSupport != null) {
                ...
                if(vertex != null && startVertex != null) {
                    DirectedSparseEdge arete = new DirectedSparseEdge(startVertex, vertex);
                    if (graph.getEdges().contains(arete))
                    {
                    	graph.editTransition(arete);
                    }
                    else
                    {
                    	graph.addEdge(arete);
